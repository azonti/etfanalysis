...
await Promise.all([
  ...configs.map(async config => {
    ...
    if (stderr.trim().split('\n').some(line => line.trim() && !line.trim().endsWith('it/s]') && line.trim() !== 'Matplotlib is building the font cache; this may take a moment.')) {
  ...
  ...configs.map(async config => {
    ...
    if (stderr.trim().split('\n').some(line => line.trim() && line.trim() !== 'Matplotlib is building the font cache; this may take a moment.')) {
      throw new Error(stderr);
    }
